...
#define pan_servo_min_angle 0

#define tilt_servo_max_angle 100
#define tilt_servo_min_angle 0
//<<This will be deleted
...
void calculate_angles(void)					//Calculate joint angles and write them to variables
{
	...
	/*float px, py, pz, tx,ty, tz, tr, tp, tya, pre_pan, pre_tilt;;
	
	px=aircraft_status.x;
	py=aircraft_status.y;
	pz=aircraft_status.z;
	
	tx=tracker_status.x;
	ty=tracker_status.y;
	tz=tracker_status.z;
	
	tr=stateGetNedToBodyEulers_f()->phi;
	tp=stateGetNedToBodyEulers_f()->theta;
	tya=stateGetNedToBodyEulers_f()->psi;
	...
	pre_pan=pre_pan*180/M_PI;
	pre_tilt=pre_tilt*180/M_PI;
			
	if (pre_pan<0) pre_pan += 360.;
	if (pre_tilt<0) pre_tilt += 360.;
...
void drive_servos(void)   					//This is just for demostratons. 
{
	...
	if (pan_angle <pan_servo_max_angle && pan_angle >0) {		//Move pan servo
					
					int pan_durt;
					pan_durt=(((PAN_SERVO_MAX-PAN_SERVO_MIN)*pan_angle)/pan_servo_max_angle)+PAN_SERVO_MIN;
					ActuatorSet(PAN_SERVO_ID, pan_durt);				
					}


	if (tilt_angle <tilt_servo_max_angle && tilt_angle >0) {	//Move tilt servo
					int tilt_durt;
...
//From this code is taken from modules/cam_control/point.c file. Read that for decumentation
...
/***        IN OTHER MODES ONLY THE CAM_POINT WAYPOINT AND THE DISTANCE FROM TARGET IS UPDATED                ***/
   ...
   if (cam_theta > RadOfDeg(tilt_servo_max_angle)){
      cam_theta = RadOfDeg(tilt_servo_max_angle);

   }else if(cam_theta < RadOfDeg(tilt_servo_min_angle)){ cam_theta = RadOfDeg(tilt_servo_min_angle); }

   
   if (cam_phi > RadOfDeg(pan_servo_max_angle)){
      cam_phi = RadOfDeg(pan_servo_min_angle);

   }else if(cam_phi < RadOfDeg(pan_servo_min_angle)){ cam_phi = RadOfDeg(pan_servo_min_angle); }
...
//cam_theta = RadOfDeg(70);     //  LOOK 45 DEGREES DOWN, 0 IS STRAIGHT DOWN 90 IS STRAIGHT IN FRONT
   ...
   if ( cam_theta > RadOfDeg(80) && mode == AUTO){ // Not much to see after 80 degrees of tilt so stop tracking.
      *fPan = cam_phi;
      *fTilt = cam_theta;
